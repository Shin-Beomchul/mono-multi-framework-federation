...
const KEYCLOAK_CLIENT_ID = 'console' // 사용할 client
const KEYCLOAK_URL = 'https://keycloak.k9s.kr' // https://keycloak.k9s.kr keycloak url
    ...
    case 'onAuthSuccess':
      ...
      if (AppConfig.authorizationType === AuthorizationType.KEYCLOAK) {
        ...
        getAuthorizationTokenAPI().then((selfAuthResponse) => {
          const authorization = { roles: selfAuthResponse.roles }
 ...
 *  ! SELF 인가는 JWT토큰이 아닐 수 있고, 서비스별로 정의된 모델을 따를 수 있지만 roles인터페이스는 반드시 필요.
 *  */
